...
- `cmd /c npm run smoke:local-ci` ผ่านแล้ว โดย local smoke บังคับ tenant topology เป็น shared และปิด bot/worker/watcher health checks ที่ไม่ได้เปิดใน stack นี้ เพื่อเลี่ยง `prisma db push --accept-data-loss`
...
- `cmd /c npm run readiness:prod -- --json --skip-smoke` ผ่าน 9 checks หลังแก้ migration wrapper และ local smoke guard โดยรวม lint, policy tests, security, doctor, topology prod และ persistence smoke
- หมายเหตุ operational: อย่ารัน `readiness:full` และ `readiness:prod` พร้อมกันในเครื่องเดียวกัน เพราะ integration tests บางชุดใช้ resource/runtime ร่วมกัน ควรรันแบบลำดับเหมือนรอบนี้
...
- Production persistence, PostgreSQL deploy path และ local smoke stack ผ่านแล้วในเครื่องนี้หลังเปิด PostgreSQL local โดย `db:migrate:deploy:postgresql` ไม่ติด `P3005` และ `smoke:local-ci` ไม่ต้องใช้ `--accept-data-loss` แล้ว แต่ยังเป็น proof ระดับ local production profile ไม่ใช่ proof ของ production/staging infra จริง
...
| 3. Database / persistence                 | 3/5   | partial        | `prisma/schema.prisma` มี PlatformTenant, Subscription, API key, Agent, Identity, Billing, Config, Restart, Notification, Audit, Raid models                                                            | local production smoke, local smoke stack และ PostgreSQL deploy wrapper ผ่านแล้ว แต่ยังต้องพิสูจน์ release-safe migration, rollback, RLS และ tenant DB topology ใน staging/production จริง | High       |
