...
export function SecurityPage({ data, onRun }) {
  const raw = data?.raw || {};
  ...
  return (
    <PageLayout
      ...
      subtitle="Security events, sessions, and audit activity."
      icon={Shield}
        ...
        >
          {securityEvents.length ? (
            <div className="space-y-3">
              {securityEvents.slice(0, 6).map((event, index) => (
                <div
                  key={event.id || index}
                ...
                >
                  ...
                  <div className="mt-1 text-xs text-zinc-400">
                    {event.detail || event.reason || event.message || "Security signal from backend"}
                  </div>
                  {event.actor ? <div className="mt-2 text-[11px] text-zinc-500">Actor: {event.actor}</div> : null}
                </div>
              ))}
            </div>
          ) : (
            <DataEmptyState
              ...
              body="Security-event rows will appear here when auth, policy, or runtime alerts are emitted."
            />
          )}
        </GlassCard>
        ...
        <div className="space-y-4">
          ...
          >
            {auditRows.length ? (
              ...
              <DataEmptyState
                title="No audit rows"
                body="The default audit query returned no rows for the current environment."
              />
            )}
          </GlassCard>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            {[
              ...
              <div key={label} className="owner-card rounded-xl border p-4">
                ...
                <div
                  className={`owner-kpi-value mt-2 text-xl font-black ${tone === "warning" ? "text-amber-300" : tone === "critical" ? "text-red-300" : "text-cyan-300"}`}
                >
                  {value}
                </div>
              </div>
