...
export function ObservabilityPage({ data, onRun }) {
  const raw = data?.raw || {};
  ...
  return (
    <PageLayout
      ...
      subtitle="Request failures, queue pressure, and delivery timing."
      icon={Activity}
    ...
    >
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard
          ...
          value={requestLogs.length}
          icon={AlertTriangle}
          sub="Backend error rows"
          spark={requestLogs.length ? "w-[60%] bg-red-400" : "w-[10%] bg-emerald-400"}
          compact
        />
        <StatCard
          label="Failed Delivery Jobs"
          value={deliverySummary.failed24h ?? deliverySummary.failed ?? 0}
          sub="Last 24 hours"
          spark="w-[18%] bg-amber-400"
          compact
        />
        <StatCard
          label="Dead-letter Queue"
          value={deliverySummary.deadLetter ?? 0}
          sub="Needs manual review"
          spark="w-[24%] bg-red-400"
          compact
        />
      </div>

      <GlassCard
        title="Backend Request Errors"
        right={<ToneBadge tone={requestLogs.length ? "warning" : "healthy"}>{requestLogs.length} rows</ToneBadge>}
      >
        {requestLogs.length ? (
            ...
            {requestLogs.slice(0, 12).map((row, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div>
                    <ToneBadge tone={status >= 500 ? "critical" : status >= 400 ? "warning" : "healthy"}>
                      {status || "OK"}
                    </ToneBadge>
                  </div>
          ...
          <DataEmptyState
            title="No request errors"
            body="The observability endpoint returned no request-error rows."
          />
        )}
      </GlassCard>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <GlassCard title="Telemetry Metrics">
          <div className="space-y-3">
            ...
            <MetricPair
              label="Error count"
              value={metrics.errors || metrics.errorCount || requestLogs.length}
