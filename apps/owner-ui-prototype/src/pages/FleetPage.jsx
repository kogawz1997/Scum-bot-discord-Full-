...
import { Bot, Cpu, Gauge, Plus, Server } from "lucide-react";
...
export function FleetPage({ data, onRun }) {
  ...
  return (
    <PageLayout
      title="Runtime Fleet"
      subtitle="Separate delivery execution from server automation runtime."
      icon={Server}
    ...
    >
      <div className="grid gap-4 md:grid-cols-4">
        <StatCard
          ...
          value={String(summary.deliveryAgentsOnline ?? 0)}
          sub={`${summary.deliveryAgentsOffline ?? 0} offline`}
          icon={Server}
        ...
        <StatCard
          label="Server Bots"
          value={String(summary.serverBotsOnline ?? 0)}
          sub={`${summary.serverBotsOffline ?? 0} offline`}
          icon={Bot}
          spark="w-[72%] bg-amber-300"
          compact
        />
        <StatCard
          label="Version Drift"
          value={String(summary.outdated ?? 0)}
          sub="Outdated runtimes"
          icon={Cpu}
          spark="w-[18%] bg-red-400"
          compact
        />
        <StatCard
          label="Provisioning"
          value="Live"
          sub="Runtime registration is connected"
          icon={Gauge}
          spark="w-[94%] bg-cyan-400"
          compact
        />
      </div>
      ...
      <GlassCard
        ...
        description="Machine binding, job handling, and announce-capable runtime nodes."
      ...
      >
        {deliveryAgents.length ? (
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-zinc-500">
                    {agent.machineName || agent.tenantName || "Machine binding pending"}
                  </div>
                </div>
                ...
                <div className={agent.latestVersion && agent.version !== agent.latestVersion ? "text-red-300" : "text-zinc-300"}>
                  ...
                  {agent.latestVersion && agent.version !== agent.latestVersion ? (
                    <span className="ml-1 text-xs text-zinc-500">to {agent.latestVersion}</span>
                  ) : null}
                </div>
          ...
          <DataEmptyState
            ...
            body="Delivery-agent runtime rows will appear here after provisioning and heartbeat registration."
          />
        )}
      </GlassCard>

      <GlassCard
        title="Server Bot Fleet"
        description="Log sync, config management, and server-side automation nodes."
      ...
      >
        {serverBots.length ? (
            ...
            {serverBots.map((bot, index) => (
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-zinc-500">{bot.machineName || "Machine binding pending"}</div>
                </div>
          ...
          <DataEmptyState
            title="No server bots"
            body="Server-bot runtime rows will appear here after provisioning and heartbeat registration."
          />
        )}
      </GlassCard>
    </PageLayout>
